                ...
                fetch(treeurl, { cache: "no-store" }).then(response => {
                    ...
                    function DownloadFiles() {
                        ...
                        fetch(`https://cdn.jsdelivr.net/gh/${ghuser}/${ghrepo}@${sha}/version.json`).then(res => res.json()).then(data => {
                            ...
                            if (branch == "beta") {
                                repoversion = parseFloat(`${latest.betaversion}`)
                                localversion = parseFloat(`${current.betaversion}`)
                            } else {
                                repoversion = parseFloat(`${latest.version}`)
                                localversion = parseFloat(`${current.version}`)
                            }
                            ...
                            function CheckVersion() {
                                return new Promise(resolve => {
                                    ...
                                    if (repoversion > localversion) {
                                        var confirmmsg
                                        if (branch == "beta") {
                                            confirmmsg = `New App Revision (BETA ${repoversion}) available! Do you want to download it?`
                                        } else {
                                            confirmmsg = `New App Revision (${repoversion}) available! Do you want to download it?`
                                        }
